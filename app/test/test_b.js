...
b('Asynchronous benchmark').run(10, function(i, done) {
    let myPromiseToxi = new Promise((resolve, reject) => {
        let sim = [];
    ...
    myPromiseToxi.then((sim, cc, time) => {
...
b('Asynchronous benchmark - Not Toxidrome').run(10, function(i, done) {
    let myPromiseToxi = new Promise((resolve, reject) => {
        let sim = [];
        let child_id = "5941025bb940186d55f23a04"
        // Selection des cas condidat
        Case.find({toxidrome: "Opiode"}, function(err, cas) {
            let cc = 0
            let start = new Date().getTime();
            for (var i = 0; i < cas.length; i++) { // boucle sur les condidats potentiels
